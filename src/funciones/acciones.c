...
void actualizarLabelTurno(GtkWidget *labelTurno, int turno) {
    gchar *texto;
    if (turno == JUGADOR1) {
        texto = g_strdup("<span font='20' weight='bold'>Turno del Jugador 1</span>");
    } else {
        texto = g_strdup("<span font='20' weight='bold'>Turno del Jugador 2</span>");
    }
    ...
    gtk_css_provider_load_from_data(provider,
                                                                    "* { padding: 10px; font-weight: bold; }", -1, NULL);
...
void on_casilla_clicked(GtkWidget *casilla, gpointer data) {
  ...
  if (datos->botonSeleccionado != NULL && datos->botonSeleccionado != casilla) {
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(datos->botonSeleccionado))) {
...
void desplegarMovimientosGUI(GtkWidget *grid, int x, int y, Pieza *pieza,
  ...
  if (pieza != NULL) {
    ...
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(button))) {
      if (movimientos == NULL) {
        movimientos = obtenerMovimientosArray(tablero, pieza, piezasBlancas, piezasNegras);
