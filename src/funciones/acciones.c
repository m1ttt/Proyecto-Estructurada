...
void agregarMovimiento(int x, int y) {
  if (x >= 0 && x < 8 && y >= 0 &&
      y < 8) { // Verifica que el movimiento esté dentro del tablero
...
void obtenerMovimientos(Pieza p) {
  ...
  case 'Q': // Reina
    for (int i = 1; i < 8;
         i++) { // Movimientos horizontales, verticales y diagonales
  ...
  case 'C': // Caballo
    ...
    for (int i = 0; i < 8; i++) {
      agregarMovimiento(p.coordenadaX + knightMoves[i][0],
                        p.coordenadaY + knightMoves[i][1]);
    }
    break;
  }
}
